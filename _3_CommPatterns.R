...
# Initial colinearity check: slope + roughness 0.9
geoCorr <- meta %>%
...
## Evaluate GAM
# NO GAIN if including roughness
...
# GAM: terrain + landform 
density.lf <- gam(
  reformulate(
    c("landform", paste0("s(", modelVars, ")")),
    response = "logDensity"),
  data = modelData, method="REML")
summary(density.lf)
...
plot4 = ggplot(scores.rda, aes(x = RDA1_align, y = RDA2_align, color=landform)) +
  #geom_jitter(size=2.8, alpha=0.8, width = 0.3, height = 0.2) + 
...
plot6 <- allComp.env %>%
  ...
  mutate(variable=factor(variable, levels = rev(c(
    "slope","aspect","profileCurvature","planCurvature")))) %>%
