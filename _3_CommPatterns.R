...
# Set correct order
matLevels <- c(
  "Terrace","Steep slope","Ridges","Broad slope","Depression","Flat area","Sand ripples")
...
# SI table: densities per taxon and landform
counts.image %>%
  group_by(landform, taxon) %>%
  summarise(
    mean_m2 = mean(density_m2, na.rm=T),
    mean_10m2 = mean_m2 * 10,
    density_10m2 = round(mean_10m2, 2), .groups="drop")  %>%
 write.table("./results/densities.txt", sep="\t", row.names=F, quote=F)

####################################################

## MODELLING: TERRAIN VS DENSITIES
# Comparing landform+geoVars / geoVars only

# Initial colinearity check: slope + roughness 0.9; roughness/slope + VRM 0.6
geoCorr <- meta %>%
  dplyr::select(ends_with("_20cm")) %>%
  drop_na() %>%
  cor(method="spearman") %>%
  as.data.frame() %>%
...
# Does roughness contribute independent explanatory power?
counts.check <- counts.corr %>%
  rownames_to_column("taxa") %>%
  pivot_longer(-taxa, names_to="image", values_to="density") %>%
  left_join(
    meta %>%
      rownames_to_column("image") %>%
      dplyr::select(image, slope_20cm, roughness_20cm), 
    by="image") %>%
  filter(density > 0)

# Run GAM
gam.check <- counts.check %>%
  group_by(taxa) %>% group_modify(~{
    m_r <- gam(density ~ s(roughness_20cm, k=4), data = .x, method="REML")
    m_s <- gam(density ~ s(slope_20cm, k=4), data = .x, method="REML")
    m_rs <- gam(density ~ s(roughness_20cm, k=4) + s(slope_20cm, k=4), data = .x, method="REML")
    tibble(
      AIC_r  = AIC(m_r),  AIC_s  = AIC(m_s), AIC_rs = AIC(m_rs),
      delta_rs = AIC_rs - pmin(AIC_r, AIC_s))}) %>%
  ungroup()

## Evaluate GAM
# NO GAIN when including roughness
# Removed subsequently
gam.check %>% summarise(
  taxa_total = n(),
  both_better = sum(delta_rs < -2, na.rm=T),
  no_gain = sum(delta_rs >= -2, na.rm=T))
...
modelData <- counts.image %>%
  dplyr::select(
    ...
    landform,
    dive,
    any_of(colnames(geoVars)[colnames(geoVars) !="roughness_20cm"])) %>%
  mutate(
    landform = fct_relevel(factor(landform),"Flat area"),
    dive = factor(dive))
...
# Confirm no further collinearities 
vif(lm(logDensity ~ ., data = modelData %>% dplyr::select(logDensity, all_of(modelVars))))

# GAM: terrain + landform; dive as random effect 
density.lf <- gam(
  reformulate(
    c("landform", paste0("s(", modelVars, ")"), "s(dive, bs='re')"),
    response = "logDensity"),
  data = modelData,
  method = "REML")

# GAM: terrain-only; dive as random effect
density.geo <- gam(
  reformulate(
    c(paste0("s(", modelVars, ")"), "s(dive, bs='re')"),
    response = "logDensity"),
  data = modelData,
  method = "REML")
...
bind_rows(
  ...
  arrange(p.value) %>%
  add_column(model="landform+terrain", .before=1) %>% 
  write.table("./results/gamLandform.txt", sep="\t", row.names=F, quote=F)
...
broom::tidy(density.geo) %>%
  ...
  write.table("./results/gamGeo.txt", sep="\t", row.names=F, quote=F)
...
# Match counts to env.rda rows
counts.sub <- counts.log[rownames(counts.log) %in% rownames(env.rda), , drop=F]
env.rda <- env.rda[rownames(counts.sub), , drop=F]

# Check for zero-variance cols
varZero <- apply(env.rda, 2, var, na.rm=T)
print(varZero)
# none -- no filtering needed

# Run RDA
rda <- rda(counts.sub ~ ., data = env.rda)
...
# Extract site scores
# Join landform info for coloring
# Rotate: visual alignment with PCA
scores.rda <- scores(rda, display="sites", scaling=2) %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Sample") %>%
  left_join(meta %>% tibble::rownames_to_column("Sample"), by="Sample") %>%
  mutate(
    RDA1_align = -RDA2,  # horizontal mirror after 90° clockwise
    RDA2_align = -RDA1)   # vertical stays flipped with rotation

# Extract variance explained
eig.rda <- summary(rda)$cont$importance[2, 1:2] * 100

# PLOT -- FIG 3c
plot4 = ggplot(scores.rda, aes(x = RDA1_align, y = RDA2_align, color=landform)) +
  geom_point(size=2.44, alpha=0.8) + 
  scale_color_manual(values=landformCol) +
  scale_shape_manual(values = c(15,17,19)) + 
  labs(x = paste0("RDA (", round(eig.rda[2], 1), "%)"),
       y = paste0("RDA (", round(eig.rda[1], 1), "%)")) +
  theme_classic() +
...
dbrda <- capscale(
  counts.dist ~ ., data = as.data.frame(env.rda))
...
# Furthermore, less dominance by "extreme" sand ripples
...
anova(rda, by="terms") %>%                       
  ...
  drop_na(p.value) %>%
  arrange(p.value) %>%                                 
  write.table("./results/rda.txt", sep="\t", row.names=F, quote=F)
...
# PCA on predictor groups to reduce collinearity
...
plot(
  ...
  Xnames = c("Depth","Geomorphology","Landform","Site"))
...
## Detailed tax-env differences
# Define priority landform pair (manual order)
sitePrio <- c("Terrace","Steep slope")
siteAll <- sort(unique(c(dunn.tax$group1, dunn.tax$group2, dunn.geo$group1, dunn.geo$group2)))
...
# Process ENV 
allComp.env <- dunn.geo %>%
  filter(p.adj < 0.05) %>%                        
  filter(str_detect(variable, "_20cm$")) %>%     
...
allComp.order <- allComp.tax %>%
  distinct(landform_pair, landformA, landformB) %>%
  arrange(factor(landformA, levels = siteLevels),
          factor(landformB, levels = siteLevels)) %>%
  pull(landform_pair)

# Apply to data
allComp.tax <- allComp.tax %>%
  mutate(landform_pair_fct = factor(landform_pair, levels = allComp.order))
allComp.env <- allComp.env %>% mutate(
...
# Compute global max
maxVal <- max(
  -log10(allComp.tax$p.adj),
  -log10(allComp.env$p.adj),
  na.rm = TRUE)

# Plot ENV
# omit roughness due to colinearity
plot6 <- allComp.env %>%
  filter(variable != "roughness") %>%    
  mutate(variable=factor(variable, levels = rev(c(
    "slope","VRM","aspect_N","aspect_E","profileCurvature","planCurvature")))) %>%
  ggplot(aes(x = landform_pair_fct, y = variable)) +
  geom_point(aes(size = -log10(p.adj), color = -log10(p.adj)), alpha = 0.8) +
  scale_size_continuous(range = c(2, 7), limits = c(0, maxVal)) +
  scale_color_viridis_c(option = "magma", direction = -1, limits = c(0, maxVal)) +
  theme_minimal() +
  theme(
    axis.title = element_blank(),
    axis.text.x = element_blank(),
...
plot7 <- allComp.tax %>%
  ...
  mutate(taxon=factor(taxon, levels = c(
    ...
    "Bryozoans","Anemones","Echinoids","Asteroids","Crinoids"))) %>%
  ggplot(aes(
    x = landform_pair_fct,
    y = fct_rev(factor(taxon)))) +
  geom_point(aes(size = -log10(p.adj), color = -log10(p.adj)), alpha = 0.8) +
  scale_size_continuous(range = c(2, 7), limits = c(0, maxVal)) +
  scale_color_viridis_c(option = "magma", direction = -1, limits = c(0, maxVal)) +
  theme_minimal() +
