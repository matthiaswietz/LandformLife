...
oce3d <- tidync("./envData/cmems_mod_glo_phy_my_0.083deg_P1M.nc") %>%
...
oce2d <- tidync("./envData/cmems_mod_glo_phy_my_0.083deg_P1M.nc") %>%
  activate("D2,D1,D0") %>%  
...
chl <- tidync("./envData/cmems_obs-oc_glo_bgc-plankton_my_l4-multi-4km_P1M.nc") %>%
  activate("D2,D1,D0") %>%  
...
merge(pca.hydro, hydro.summary[, .(regime, regime_new, density)], by="regime") %>%
  ...
  geom_label(
    ...
    linewidth=0.1, label.padding=unit(0.10, "lines"),
...
dunn.hydro <- hydroVars[hydroVars %in% arrow$Var] %>%
  ...
  mutate(
    ...
    PC_rank = rank(-abs_PC1),
    effsize_rank = rank(-mean_effsize),
    combined_rank = round((effsize_rank + PC_rank) / 2)) %>%
...
dunn.hot <- hotspots %>%
  ...
  mutate(zone = case_when(
    ...
    as.vector(hot_global) & as.vector(hot_depth) ~ "High-density cluster",
    TRUE ~ "Non-Hotspot")) %>%
  group_by(zone) %>%
...
hot.summary <- {
  vars_keep <- dunn.hot %>%
    ...
    distinct(variable) %>%
    pull(variable) %>%
    setdiff(grep("cor|demo|glass|ophi|sea", ., value=T))
  df0 <- hotspots %>%
    dplyr::select(
      x, y, xr, yr,
