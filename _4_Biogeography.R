...
# Define order
varOrder <- c("slope","roughness","VRM","aspect_N","aspect_E","profileCurvature","planCurvature")
resOrder <- c("20cm","25m")
...
resCorr <- meta %>%
  select(matches("^(VRM|aspect_E|aspect_N|planCurvature|profileCurvature|roughness|slope)_(20cm|25m)$")) %>%
  drop_na() %>%
  cor(method="spearman") %>%
  as.data.frame() %>%
  ...
  filter(var1 < var2) %>%
  filter(
    sub(".*_(20cm|25m)$", "\\1", var1) !=
    sub(".*_(20cm|25m)$", "\\1", var2)) %>%
...
ggplot(aes(x = loRes_ord, y = hiRes_ord)) +
  ...
  scale_fill_gradientn(
    colors = c("aliceblue","skyblue1","skyblue3","dodgerblue4","gray22","gray2"),
    limits = c(0, 0.8)) +
  scale_shape_manual(
    values = c("pos"=24,"neg"=25),
    labels = c("Positive","Negative")) +
  scale_size(range = c(1.4, 5.5)) +
  guides(size="none") +
  theme_classic(base_size=11) +
  theme(
    panel.grid.major = element_line(linewidth=0.1, color="gray84"),
...
predict.hires <- meta %>%
  ...
  drop_na() %>%
  rename_with(~ stringr::str_remove(.x, "_[0-9]+(cm|m)")) %>%
  #mutate(across(where(is.numeric), ~ as.vector(scale(.x)))) %>%
  scale() %>%
  as.data.frame() %>%
...
predict.lowres <- meta %>%
  ...
  rename_with(~ stringr::str_remove(.x, "_[0-9]+(m)")) %>%
  drop_na() %>%
  scale() %>%
  as.data.frame() %>%
  dplyr::select(-c("roughness"))
...
mutate(
  ...
  scale_color_manual(values = c(
    "OFOBS"="deepskyblue4","Ship"="lightcoral"), name="Bathymetry") +
  ...
  theme(
    ...
    panel.grid.major = element_line(linewidth=0.2, color="gray84"))

## Summary: significance & effect size
rbind(stats.hires, stats.lowres) %>%
  filter(Significant == "TRUE" & Predictor=="slope") %>%
...
ps118_slope <- rast("./envData/ps118_slope_25m.tif")
...
# Deviance explained
data.frame(
  Taxon = c("Coral","Demosponges","Glass sponges","Ophiuroids","Sea pens"),
  Deviance_Percent = c(
...
writeRaster(c(
  ...
  "~/Rdata/bioPredict.tif", filetype="GTiff", overwrite=T)
