...
# Identify "hot" grid cells (90% density quantiles)
# Both global and by depth strata (500m bins)
...
for(type in c("hot_depth","hot_global")) {
  ...
  knn_all <- FNN::get.knnx(
    ...
    query = as.matrix(hotspots[, c("xr","yr")]), k = 1)
...
if(length(idx) > 0){
    knn_hot <- FNN::get.knnx(
      data  = as.matrix(grid[, c("xr","yr")]),
      query = as.matrix(hotspots[idx, c("xr","yr")]), k = 1)
...
hottest <- hotspots %>%
  ...
  filter(strength_hot_global >0.84 & strength_hot_depth >0.88) %>% 
  ...
  left_join(
    {df <- .
      cent <- df %>%
        group_by(Hotspot) %>%
        summarise(cx = mean(x, na.rm=T), cy = mean(y, na.rm=T), .groups="drop")
      east_ids <- cent %>%
  ...
  bind_rows(map_east, map_rest) %>%
        select(Hotspot, hotspot_id)}, by="Hotspot") 
...
# Lookup for later checks
hot.lookup <- hotspots %>%
  select(x, y, hot_global, hot_depth) %>%
  left_join(
    hottest %>% select(x, y, Hotspot),
    by = c("x", "y")) %>%
  mutate(
    zone = case_when(
      !is.na(Hotspot) ~ "Hotspot",
      as.logical(hot_global) & as.logical(hot_depth) ~
        "High-density cluster", TRUE ~ "Non-Hotspot"),
